...
use crate::{authenticated_transport::AuthenticatedTransportConnect, bench_mode::BenchMode, bsc_config};
...
impl BenchContext {
    ...
    pub(crate) async fn new(bench_args: &BenchmarkArgs, rpc_url: String) -> eyre::Result<Self> {
        ...
        // Check if this is a BSC chain by checking the chain ID
        let chain_id = block_provider.get_chain_id().await?;
        let is_bsc = bsc_config::is_bsc_chain(chain_id);
        
        if is_bsc {
            info!("Detected {} (Chain ID: {})", bsc_config::bsc_network_name(chain_id), chain_id);
        }
        
        // Check if this is an OP chain by checking code at a predeploy address.
        let is_optimism = !is_bsc && !block_provider
        ...
        Ok(Self { auth_provider, block_provider, benchmark_mode, next_block, is_optimism, chain_id })
    }
}
